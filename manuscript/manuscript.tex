...
\begin{thm}
  Every equilibrium points $P$
  except the stabilizing one are unstable.
\end{thm}

\begin{lem}
  If $(\tilde{H}_k)_{22} \succ 0$, then $\rho(A_k)$ is bounded.
\end{lem}
...
\begin{proof}
  ...
  Then,
  \begin{equation}
    K_k v = (B^T R^{-1} B)^{-1} B^T R^{-1} (A - \lambda I)v.
  \end{equation}
  Let $v = B w + z$,
  where $z \in \ker(B^T)$.
  Then,
  \begin{equation}
    w = (B^T R^{-1} B)^{-1} B^T R^{-1} (v - z).
  \end{equation}
  Let
  \begin{align}
    C & = R (B^T R^{-1} B)^{-1} B^T R^{-1},
    \\
    D & = C^T R^{-1} C.
  \end{align}
\end{proof}
